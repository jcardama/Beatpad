...
- More hotkeys: a **panic** key (default Space) stops every running loop at once, **Ctrl/⌘+Backspace** clears the board, and the global play-mode keys ship default bindings (9 / 0 / -). All remappable in Settings → Keybindings.
...
- Dropped the empty placeholder buttons framing the board, leaving only the controls that do something (bank toggle, play-mode buttons, logo).
